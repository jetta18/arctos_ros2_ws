...
STM32ProtocolV2::STM32ProtocolV2(int socket_fd, rclcpp::Logger & logger)
...
bool STM32ProtocolV2::send_command(
  uint8_t command, const void * payload, uint16_t payload_len)
{
  ...
  uint8_t buffer[ProtocolV2Constants::MAX_PACKET_SIZE];
  Header header;
  header.version = ProtocolV2Constants::PROTOCOL_VERSION;
...
bool STM32ProtocolV2::receive_response(
  ...
  uint16_t max_payload, uint16_t & payload_len_out)
{
  ...
  if (header_out.version != ProtocolV2Constants::PROTOCOL_VERSION)
  {
    return false;
  }
...
bool STM32ProtocolV2::send_and_expect_ok(
  uint8_t command, const void * payload, uint16_t payload_len)
{
  ...
  if (resp.response != ProtocolV2Constants::RESP_OK)
  {
    if (resp.response == ProtocolV2Constants::RESP_ERROR && resp_len >= 32)
    {
      char msg[32] = {};
...
bool STM32ProtocolV2::ping()
{
  for (int attempt = 0; attempt < ProtocolV2Constants::PING_MAX_ATTEMPTS; ++attempt)
  {
    if (send_and_expect_ok(ProtocolV2Constants::CMD_PING, nullptr, 0))
    {
      return true;
    }
    std::this_thread::sleep_for(ProtocolV2Constants::PING_RETRY_DELAY);
  }
  return false;
}

bool STM32ProtocolV2::stop()
{
  return send_and_expect_ok(ProtocolV2Constants::CMD_STOP, nullptr, 0);
}

bool STM32ProtocolV2::read_state(StateResponse & state_out)
{
  if (!send_command(ProtocolV2Constants::CMD_GET_STATE, nullptr, 0))
  {
    return false;
  }
  ...
  if (resp.response != ProtocolV2Constants::RESP_STATE)
  {
    return false;
  }
  ...
  {
    float positions[ProtocolV2Constants::MAX_JOINTS];
    float velocities[ProtocolV2Constants::MAX_JOINTS];
  ...
  for (size_t i = 0; i < ProtocolV2Constants::MAX_JOINTS; ++i)
  {
...
bool STM32ProtocolV2::trajectory_begin(uint32_t trajectory_id, uint16_t num_points)
{
  ...
  return send_and_expect_ok(
    ProtocolV2Constants::CMD_TRAJ_BEGIN, &payload, sizeof(payload));
}

bool STM32ProtocolV2::trajectory_send_point(
  ...
  const TrajectoryPoint & point)
{
  ...
  bool ok = send_command(
    ProtocolV2Constants::CMD_TRAJ_POINT, &payload, sizeof(payload));
  std::this_thread::sleep_for(std::chrono::microseconds(500));
  return ok;
}

bool STM32ProtocolV2::trajectory_execute(uint32_t trajectory_id)
{
  struct __attribute__((packed))
  {
    uint32_t trajectory_id;
  ...
  return send_and_expect_ok(
    ProtocolV2Constants::CMD_TRAJ_EXECUTE, &payload, sizeof(payload));
}

bool STM32ProtocolV2::trajectory_cancel()
{
  return send_and_expect_ok(ProtocolV2Constants::CMD_TRAJ_CANCEL, nullptr, 0);
}
